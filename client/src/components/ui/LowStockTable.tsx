import type { LowStockItem } from '@/types/overview';

type LowStockTableProps = {
  items: LowStockItem[];
};

export default function LowStockTable({ items }: LowStockTableProps) {
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-4">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
          Inventory Risk
        </p>
        <h2 className="mt-2 text-2xl font-bold tracking-[-0.02em] text-slate-900 dark:text-white">
          Low stock items
        </h2>
      </div>

      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-800">
              <th className="py-3 pr-4 text-left font-semibold text-slate-500 dark:text-slate-400">
                Product
              </th>
              <th className="py-3 pr-4 text-left font-semibold text-slate-500 dark:text-slate-400">
                SKU
              </th>
              <th className="py-3 pr-4 text-left font-semibold text-slate-500 dark:text-slate-400">
                Stock
              </th>
              <th className="py-3 text-left font-semibold text-slate-500 dark:text-slate-400">
                Reorder
              </th>
            </tr>
          </thead>
          <tbody>
            {items.map((item) => (
              <tr key={item.product_id} className="border-b border-slate-100 dark:border-slate-800/70">
                <td className="py-4 pr-4 font-normal text-slate-800 dark:text-slate-100">
                  {item.product_name}
                </td>
                <td className="py-4 pr-4 text-slate-500 dark:text-slate-400">{item.sku}</td>
                <td className="py-4 pr-4">
                  <span className="rounded-full bg-red-50 px-2.5 py-1 text-xs font-bold text-red-600 dark:bg-red-950/50 dark:text-red-300">
                    {item.stock_on_hand}
                  </span>
                </td>
                <td className="py-4 text-slate-700 dark:text-slate-300">{item.reorder_level}</td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}