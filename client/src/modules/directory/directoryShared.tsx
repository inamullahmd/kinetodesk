...
import type { DirectoryPagination } from './directory.types'
import {
  formatEnumLabel,
...
export function WorkflowStatusBadge({
  ...
  const className =
    ['fulfilled', 'delivered', 'paid'].includes(normalized)
      ? isDark
        ? 'bg-emerald-500/10 text-emerald-300 ring-emerald-500/20'
        : 'bg-emerald-50 text-emerald-700 ring-emerald-200'
      : ['issued', 'transit', 'pending', 'confirmed', 'shipped'].includes(normalized)
        ? isDark
          ? 'bg-blue-500/10 text-blue-300 ring-blue-500/20'
          : 'bg-blue-50 text-blue-700 ring-blue-200'
        : ['cancelled', 'returned', 'refunded'].includes(normalized)
          ? isDark
  ...
  return (
    <span className={`inline-flex whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-semibold leading-none ring-1 ring-inset ${className}`}>
      {formatEnumLabel(status)}
    </span>
  )
}
  ...
  return (
    <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
      <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
        ...
        <span className="font-semibold">{pagination.total}</span> {label}
      </p>
